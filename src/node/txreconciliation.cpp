...
public:
    ...
    std::unordered_set<NodeId> GetInboundFanoutTargets() const EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex) {
        AssertLockNotHeld(m_txreconciliation_mutex);
        // TODO: Do we need to hold it? Or only make sure it's not held
        LOCK(m_txreconciliation_mutex);
        return m_inbound_fanout_targets;
    }
...
std::unordered_set<NodeId> TxReconciliationTracker::GetInboundFanoutTargets()
{
    return m_impl->GetInboundFanoutTargets();
}
