...
public:
    ...
    void GetRelevantIDsFromShortIDs(const std::vector<uint64_t>& diff,
                                    // returning values
                                    std::vector<uint32_t>& local_missing, std::vector<Wtxid>& remote_missing, bool from_snapshot) const
    {
    ...
    std::vector<Wtxid> GetWtxidsFromShortIDs(const std::vector<uint32_t>& remote_missing_short_ids) const
    {
        ...
        for (const auto& missing_short_id : remote_missing_short_ids) {
            const auto local_tx = m_snapshot_short_id_mapping.find(missing_short_id);
            if (local_tx != m_snapshot_short_id_mapping.end()) {
                remote_missing.push_back(local_tx->second);
            }
        }
...
private:
                             ...
                             std::vector<uint32_t>& txs_to_request, std::vector<Wtxid>& txs_to_announce, std::optional<bool>& result)
    {
        ...
        if (!remote_sketch.has_value() || !local_sketch.has_value()) {
            ...
            if (local_sketch.value().Merge(remote_sketch.value()).Decode(differences)) {
                ...
                recon_state.GetRelevantIDsFromShortIDs(differences, txs_to_request, txs_to_announce, /*from_snapshot=*/false);
                // Update local reconciliation state for the peer.
                recon_state.FinalizeInitByUs(true);
                recon_state.m_phase = Phase::NONE;
    ...
    bool HandleSketchExtension(TxReconciliationState& recon_state, const NodeId peer_id,
        ...
        if (local_sketch.Merge(remote_sketch).Decode(differences)) {
            ...
            // Identify locally/remotely missing transactions.
            recon_state.GetRelevantIDsFromShortIDs(differences, txs_to_request, txs_to_announce, /*from_snapshot=*/true);
...
public:
    ...
    bool ShouldRespondToReconciliationRequest(NodeId peer_id, std::vector<uint8_t>& skdata, bool send_trickle) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        if (incoming_phase == Phase::INIT_REQUESTED && send_trickle) {
        ...
        const std::vector<uint32_t>& remote_missing_short_ids, std::vector<Wtxid>& remote_missing) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        // Update local reconciliation state for the peer.
        recon_state.ClearLocalSetSnapshot();
        recon_state.m_phase = Phase::NONE;
