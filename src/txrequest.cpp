...
struct Announcement {
    ...
    /** The first time we heard about this transaction. This time will match for all announcements of the same transaction. */
    std::chrono::microseconds m_first_heard_of;
...
public:
        ...
        std::chrono::microseconds recvtime, std::chrono::microseconds delay)
    {
        ...
        auto it = m_index.get<ByTxHash>().lower_bound(ByTxHashView{txhash, State::CANDIDATE_DELAYED, 0});
        auto first_heard_of = (it != m_index.get<ByTxHash>().end() && it->m_txhash == txhash) ? it->m_first_heard_of : recvtime;
        auto ret = m_index.get<ByPeer>().emplace(gtxid, peer, preferred, first_heard_of, recvtime+delay, m_current_sequence);
