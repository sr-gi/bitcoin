...
BOOST_FIXTURE_TEST_CASE(improves_feerate, TestChain100Setup)
{
    ...
    changeset->StageAddition(tx1_conflict, tx1_fee, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
    changeset->StageAddition(tx3, tx2_fee, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
    ...
    changeset->StageRemoval(entry1);
    changeset->StageRemoval(entry2);
    changeset->StageAddition(tx1_conflict, tx1_fee+1, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
    changeset->StageAddition(tx3, tx2_fee, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
    ...
    changeset->StageRemoval(entry2);
    changeset->StageAddition(tx1_conflict, tx1_fee, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
    changeset->StageAddition(entry4.GetSharedTx(), tx2_fee, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
    BOOST_CHECK(ImprovesFeerateDiagram(*changeset) == std::nullopt);
    changeset.reset();
    ...
    changeset->StageAddition(tx1_conflict, tx1_fee, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
    changeset->StageAddition(entry4.GetSharedTx(), tx2_fee + entry5->GetModifiedFee() + 1, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
...
BOOST_FIXTURE_TEST_CASE(calc_feerate_diagram_rbf, TestChain100Setup)
{
    ...
    // Replacement of size 1
    {
        ...
        changeset->StageAddition(replacement_tx, 0, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
    ...
    // Non-zero replacement fee/size
    {
        auto changeset = pool.GetChangeSet();
        changeset->StageRemoval(entry_low);
        changeset->StageAddition(replacement_tx, high_fee, 0, 1, 0, false, 4, LockPoints(), entry.m_consider_fanout);
