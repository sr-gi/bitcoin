...
BOOST_AUTO_TEST_CASE(ShouldFanoutToTest)
{
    ...
    NodeId peer_id0 = 0;
    FastRandomContext frc{/*fDeterministic=*/true};
    std::vector<NodeId> in_fanout_targets, out_fanout_targets;
    ...
    for (int i = 0; i < 100; ++i) {
        std::tie(in_fanout_targets, out_fanout_targets) = tracker.GetFanoutTargets(Wtxid::FromUint256(frc.rand256()), /*inbounds_fanout_tx_relay=*/0, /*outbounds_fanout_tx_relay=*/0);
        BOOST_CHECK(tracker.ShouldFanoutTo(peer_id0, in_fanout_targets, out_fanout_targets));
    }

    // Don't select a fanout target if we already have enough targets
    for (int i = 0; i < 100; ++i) {
        std::tie(in_fanout_targets, out_fanout_targets) = tracker.GetFanoutTargets(Wtxid::FromUint256(frc.rand256()), /*inbounds_fanout_tx_relay=*/0, /*outbounds_fanout_tx_relay=*/1);
        BOOST_CHECK(!tracker.ShouldFanoutTo(peer_id0, in_fanout_targets, out_fanout_targets));
    }
    ...
    std::vector<NodeId> in_fanout_targets_t2, out_fanout_targets_t2;
    std::tie(in_fanout_targets, out_fanout_targets) = tracker.GetFanoutTargets(Wtxid::FromUint256(wtxid), /*inbounds_fanout_tx_relay=*/0, /*outbounds_fanout_tx_relay=*/0);
    std::tie(in_fanout_targets_t2, out_fanout_targets_t2) = tracker2.GetFanoutTargets(Wtxid::FromUint256(wtxid), /*inbounds_fanout_tx_relay=*/0, /*outbounds_fanout_tx_relay=*/0);
    for (int i = 1; i < inbound_peers; ++i) {
        total_fanouted1 += tracker.ShouldFanoutTo(i, in_fanout_targets, out_fanout_targets);
        total_fanouted2 += tracker2.ShouldFanoutTo(i, in_fanout_targets_t2, out_fanout_targets_t2);
    }
    ...
    std::tie(in_fanout_targets, out_fanout_targets) = tracker.GetFanoutTargets(Wtxid::FromUint256(frc.rand256()), /*inbounds_fanout_tx_relay=*/4, /*outbounds_fanout_tx_relay=*/0);
    for (int j = 0; j < 100; ++j) {
        size_t total_fanouted = 0;
        for (int i = 1; i < inbound_peers; ++i) {
            total_fanouted += tracker.ShouldFanoutTo(i, in_fanout_targets, out_fanout_targets);
        }
