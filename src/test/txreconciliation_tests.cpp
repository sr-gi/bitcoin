...
BOOST_AUTO_TEST_CASE(ForgetPeerTest)
{
    ...
    BOOST_CHECK_EQUAL(tracker.RegisterPeer(peer_id0, true, TXRECONCILIATION_VERSION, 1), ReconciliationRegisterResult::NOT_FOUND);
...
BOOST_AUTO_TEST_CASE(IsPeerNextToReconcileWithTest)
{
    ...
    peer1_next = tracker.IsPeerNextToReconcileWith(peer_id1, current_time);
    peer2_next = tracker.IsPeerNextToReconcileWith(peer_id2, current_time);
    BOOST_CHECK(peer2_skipped && peer1_next && !peer2_next);
    ...
    BOOST_CHECK(peer1_next && peer1_next_next);
    ...
    BOOST_CHECK(!peer2_next_next && !peer2_next_next_next);
}
...
BOOST_AUTO_TEST_CASE(InitiateReconciliationRequestTest)
{
    ...
    BOOST_CHECK(reconciliation_request_params.has_value());
    auto [local_set_size, local_q_formatted] = reconciliation_request_params.value();
    BOOST_CHECK_EQUAL(local_set_size, 0);
    BOOST_CHECK_EQUAL(local_q_formatted, uint16_t(Q * Q_PRECISION));
    ...
    BOOST_CHECK(!tracker.InitiateReconciliationRequest(peer_id1).has_value());
    ...
    auto n_added_txs = frc.randrange(42) + 1;
    for (auto i=0; i<n_added_txs; i++) {
    ...
    BOOST_CHECK_EQUAL(local_set_size, n_added_txs);
    BOOST_CHECK_EQUAL(local_q_formatted, uint16_t(Q * Q_PRECISION));
...
BOOST_AUTO_TEST_CASE(HandleReconciliationRequestTest)
{
    ...
    // A reconciliation request cannot be initiated with a non-fully registered peer
    BOOST_CHECK(!tracker.HandleReconciliationRequest(peer_id0, 0, Q));
    tracker.PreRegisterPeer(peer_id0);
    BOOST_CHECK(!tracker.HandleReconciliationRequest(peer_id0, 0, Q));
    ...
    tracker.PreRegisterPeer(peer_id1);
    BOOST_REQUIRE_EQUAL(tracker.RegisterPeer(peer_id1, /*is_peer_inbound*/true, TXRECONCILIATION_VERSION, 1), ReconciliationRegisterResult::SUCCESS);
    BOOST_CHECK(tracker.HandleReconciliationRequest(peer_id1, 0, Q));
    BOOST_CHECK(!tracker.HandleReconciliationRequest(peer_id1, 0, Q));

    // We only accept requests if they are the initiator (peer is inbound)
    NodeId peer_id2 = 2;
    tracker.PreRegisterPeer(peer_id2);
    ...
    BOOST_CHECK(!tracker.HandleReconciliationRequest(peer_id2, 0, Q));
}

BOOST_AUTO_TEST_CASE(ShouldRespondToReconciliationRequest) {
    TxReconciliationTracker tracker(TXRECONCILIATION_VERSION, INBOUND_FANOUT_DESTINATIONS_FRACTION, OUTBOUND_FANOUT_THRESHOLD);
    NodeId peer_id0 = 0;
    FastRandomContext frc{/*fDeterministic=*/true};
    ...
    BOOST_CHECK(!tracker.ShouldRespondToReconciliationRequest(peer_id0, skdata, /*send_trickle*/true));
    tracker.PreRegisterPeer(peer_id0);
    BOOST_CHECK(!tracker.ShouldRespondToReconciliationRequest(peer_id0, skdata, /*send_trickle*/true));
    ...
    BOOST_CHECK(!tracker.ShouldRespondToReconciliationRequest(peer_id0, skdata, /*send_trickle*/true));
    BOOST_CHECK(tracker.HandleReconciliationRequest(peer_id0, /*peer_recon_set_size*/10, Q));

    // For the initial request, we only respond on trickle
    BOOST_CHECK(!tracker.ShouldRespondToReconciliationRequest(peer_id0, skdata, /*send_trickle*/false));
    BOOST_CHECK(tracker.ShouldRespondToReconciliationRequest(peer_id0, skdata, /*send_trickle*/true));

    // The sketch data depends on the transactions we had on the perr's local set
    BOOST_CHECK(skdata.empty());

    // After responding, the reconciliation phase have moved from INIT_REQUESTED to INIT_RESPONDED, so we cannot
    // respond again
    BOOST_CHECK(!tracker.ShouldRespondToReconciliationRequest(peer_id0, skdata, /*send_trickle*/true));
    ...
    tracker.PreRegisterPeer(peer_id0);
    BOOST_REQUIRE_EQUAL(tracker.RegisterPeer(peer_id0, /*is_peer_inbound*/true, TXRECONCILIATION_VERSION, 1), ReconciliationRegisterResult::SUCCESS);
    BOOST_CHECK(tracker.HandleReconciliationRequest(peer_id0, /*peer_recon_set_size*/0, Q));

    // If the received request had size zero (peer_recon_set_size == 0), we will respond but shortcut, since reconciling
    // is pointless
    tracker.AddToSet(peer_id0, Wtxid::FromUint256(frc.rand256()));
    BOOST_CHECK(tracker.ShouldRespondToReconciliationRequest(peer_id0, skdata, /*send_trickle*/true));
    BOOST_CHECK(skdata.empty());
    ...
    NodeId peer_id1 = 1;
    tracker.PreRegisterPeer(peer_id1);
    BOOST_REQUIRE_EQUAL(tracker.RegisterPeer(peer_id1, /*is_peer_inbound*/false, TXRECONCILIATION_VERSION, 1), ReconciliationRegisterResult::SUCCESS);
    BOOST_CHECK(!tracker.ShouldRespondToReconciliationRequest(peer_id1, skdata, /*send_trickle*/true));
