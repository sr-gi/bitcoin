...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-blackhole", "Test-only (INSECURE): act as a blackhole. Receive and process all data normally (download, update filters) but forward none of it: offer no blocks or transactions (outbound INV/TX/BLOCK/CMPCTBLOCK/BLOCKTXN/MERKLEBLOCK/HEADERS/NOTFOUND are dropped) and add nothing to any reconciliation set. Used to emulate an eclipse. SHOULD NOT BE USED IN PRODUCTION. (default: 0)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
