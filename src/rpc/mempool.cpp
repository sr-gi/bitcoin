    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const PackageMempoolAcceptResult package_result = [&] {
                LOCK(::cs_main);
                if (txns.size() > 1) return ProcessNewPackage(chainstate, mempool, txns, /*first_announcement_time=*/{}, /*test_accept=*/true, /*client_maxfeerate=*/{});
...
static std::vector<RPCResult> MempoolEntryDescription()
{
    return {
        ...
        RPCResult{RPCResult::Type::NUM_TIME, "announcement_time", "local time transaction was first announced to us in microseconds since 1 Jan 1970 GMT. Matches time_us if we are the origin"},
...
static void entryToJSON(const CTxMemPool& pool, UniValue& info, const CTxMemPoolEntry& e) EXCLUSIVE_LOCKS_REQUIRED(pool.cs)
{
    ...
    info.pushKV("announcement_time", count_microseconds(e.GetFirstInvTime()));
...
static RPCHelpMan submitpackage()
{
    return RPCHelpMan{"submitpackage",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const auto package_result = WITH_LOCK(::cs_main, return ProcessNewPackage(chainstate, mempool, txns, /*first_announcement_time=*/{}, /*test_accept=*/ false, client_maxfeerate));
